...
- `QWS_安妮案供词链试问台` 这一轮最稳定的新增判断，不是某句可引用原文，而是把 `供词` 这一列的程序链进一步压清了：
  - `single confession`
  - `procedural expansion`
  - `multi-person indictment`
- 这意味着，供词装置在安妮案中的真正功能，不只是提供一份口供，而是为案件第一次提供可被程序吸收的文本接口，随后再通过审问、并案和起诉书整合，把单点怀疑扩成整案。
- 因而，本页现在可以更明确地区分三种装置的技术差异：
  - `誓言` 是主动绑定
  - `供词` 是程序扩展
  - `沉默` 是解释接管
- 这一轮也同时确认了方法边界：
  - `GraphRAG global` 足以补“供词如何扩案”的中层结构
  - 但不足以安全提供 close reading 级细节
  - 所以后续若继续补 `供词` 线，应转回 `NotebookLM` 或外部来源去拿更细的场景例子，而不是继续放大全局问答
